...
const Carrousel = () => {
  const[lastHouses ,setLastHouses]= useState([])
    ...
    const settings = {
        ...
        const getLastHouses = async ()=>{
            const request = await fetch("http://localhost:5000/last-houses",{
                method:"GEt"
            })
             const response = await request.json()
            setLastHouses(response)
    ...
    return ( 
        ...
        <Slider {...settings} className="w-full h-full ">
          ...
          {lastHouses?.map((house: Houses, index: number) => (
        <div className="w-full h-full relative " key={index}>
         
          <Link href={`/houses/${house.id}`} key={index}>
            <div className="w-full h-full ">
              <img
                src={house.images[0]}
                alt={house.name}
                className="w-full h-[500px] brightness-75  "
              />
             ...
             <span className="text-white top-2 left-2 absolute font-bold bg-black bg-opacity-50 px-3 py-1 rounded-md text-sm">
              ...
              <p className="text-white text-lg absolute top-[30%] flex items-center justify-center font-bold uppercase w-full  ">
                {house.categories?.name}
              </p>
              <div className="w-full flex absolute top-[40%]  items-center justify-center gap-10 ">
        ...
        <div className="text-white w-full absolute top-[60%] py-2 px-4 rounded-md flex items-center justify-center">
              
              <span className="bg-[#302F4D] px-10 py-2 rounded-md  text-xl flex flex-col items-center">
              <p>Oportunidade Única</p>
                { (Number(house.price) / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) }
              </span>
              </div>
             </div>
